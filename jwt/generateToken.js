...
var tokenDelegationLevel = 41; //33 seem to be max with RSA //42 max with EC256 key //41 max with EC512
var tokensArray = [];
var delegatedArray = [];
...
function GenerateManyTokens(amount, delegationLevel){
    ...
    for (a = 0; a < delegationCount; a++){
        ...
        console.log(timePer); //For testing purposes it prints the time.
    }
}
...
function writeAll(path, token, i){
    ...
    var timeSeconds = Math.floor(timeMs / 1000); 
    timeMs = timeMs - (timeSeconds*1000)
    console.log("Successfully wrote", howManyTokensToMake, "token(s) at level", i, "at", timeSeconds, "s and", timeMs, "ms, to path: ", path);
    });  
}